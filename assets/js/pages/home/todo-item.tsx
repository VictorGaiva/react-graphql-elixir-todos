...
export default function TodoListItem({ item, onClose, folderId, }: { folderId: string; item?: TodoItem; onClose?: () => void; }) {
    ...
    update(cache) {
      ...
      cache.writeQuery({
        ...
        data: {
          ...
          self: {
            ...data.self,
            folders: data.self.folders.map((f) =>
              f.id === folderId ? { ...f, items: f.items.filter(({ id }) => id !== item.id) } : f
            ),
          },
        },
      });
    ...
    update(cache, { data: { createTodoItem: newTodo } }) {
      ...
      cache.writeQuery({
        ...
        data: {
          ...
          self: {
            ...data.self,
            folders: data.self.folders.map((f) => (f.id === folderId ? { ...f, items: [...f.items, newTodo] } : f)),
          },
        },
      });
    },
  ...
  function handleBlur(e: Parameters<InputProps["onBlur"]>[0]) {
    if (item) {
      if (text.trim() !== item?.content && text.trim() === "") deleteItem({ variables: { id: item?.id } });
      else updateItem({ variables: { id: item?.id, content: text.trim() } });
    } else {
      if (text.trim() === "") onClose();
      else {
        createTodoItem({ variables: { content: text.trim(), folderId } }).then(onClose);
      }
    }
  }
    ...
    if (e.key === "Enter") {
      if (item) {
        ...
        if (text.trim() === "") onClose();
          ...
          createTodoItem({
            variables: { content: text.trim(), folderId },
          }).then(() => setText(""));
      }
    }
  }
